package telemoney.resources.api.application;
 ...
 * @author Lenovo
 */
@Component(property = { JaxrsWhiteboardConstants.JAX_RS_APPLICATION_BASE + "=/resources",
		JaxrsWhiteboardConstants.JAX_RS_NAME + "=Resources.Rest" }, service = Application.class)
public class TelemoneyResourcesApiApplication extends Application {

	private static final Log LOG = LogFactoryUtil.getLog(TelemoneyResourcesApiApplication.class);
	...
	public Response getResourceDetails(@HeaderParam("language") String language, @HeaderParam("channel") Long channelId,
			String jsonResourceRequestBody) {
		long time1 = System.currentTimeMillis();
		LOG.info("------------------------- Resource API --------------------------");
		LOG.info("------------------------- Request Body --------------------------");
		LOG.info(jsonResourceRequestBody);
		LOG.info("------------------------- Header --------------------------");
		LOG.info(language);
		LOG.info(channelId);
		...
		try {
			...
			TestRequestBody resourceBodyRequest = objectMapper.readValue(jsonResourceRequestBody,
					TestRequestBody.class);
			if (resourceBodyRequest.getResourceCode() == null) {
				...
				LOG.info("Resources API Time: " + String.valueOf(System.currentTimeMillis() - time1));
			...
			body = _resourcesLocalService.getResourceDetailsApiLatestApproved(httpServletRequest, languageName, channelId, jsonResourceRequestBody);
			if (body.length() == 0) {
				header.put("statusCode", TelemoneyConstants.STATUS_CODE_UNKNOWN_RESOURCE_CODE);
				header.put("statusDescription", TelemoneyConstants.STATUS_DESC_UNKNOWN_RESOURCE_CODE);
	...
	@Reference
	ResourceLocalService _resourcesLocalService;
